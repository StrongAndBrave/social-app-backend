...
export class ProfileController {
	constructor(
		...
		@Inject(ProfileQueryRepository.name)
		private profileQueryRepository: ProfileQueryRepository,
		@Inject(AvatarQueryRepository.name)
		private readonly avatarQueryRepository: AvatarQueryRepository,
	...
	async uploadAvatar(
		...
		const newAvatars = await this.commandBus.execute(
			new AddAvatarCommand(userId, avatar.buffer),
		);
		if (!newAvatars)
