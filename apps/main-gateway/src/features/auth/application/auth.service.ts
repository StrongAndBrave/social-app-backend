...
export class AuthService {
	...
	async resendEmail(email: string): Promise<boolean | null> {
		const token = uuidv4();
		try {
			const user = await this.userRepository.getByUnique({ email: email });
			if (!user) return null;
			...
			await this.userRepository.update({
				where: { id: user.id },
				data: { confirmationCode: token, codeExpirationDate: date },
			});
	...
	async validateUser(loginOrEmail: string, password: string): Promise<string> {
		...
		if (!isPasswordEquals)
			throw UnauthorizedDomainException.create('User or password not valid');
		return user.id;
	}
